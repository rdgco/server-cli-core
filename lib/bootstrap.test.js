...
describe('bootstrap()', () => {
  ...
  describe('module init', () => {
    ...
    test('prints a clean message (no FATAL/stack) when init throws err.isClean', async () => {
      fakeModule(tmpDir, 'cleanfail', `
        export const metadata = { prefix: 'cleanfail' };
        export async function init() {
          const err = new Error('please configure me first');
          err.isClean = true;
          throw err;
        }
      `);
      ...
      await expect(
        bootstrap({ modulesDir: tmpDir, autoStartRepl: false, includePackageModules: false })
      ).rejects.toThrow('please configure me first');
      const text = errSpy.mock.calls.flat().join('\n');
      // Clean errors surface their message verbatim, without the FATAL prefix.
      expect(text).toContain('please configure me first');
  ...
  describe('dispatch chain', () => {
    ...
    test('a module without a handle function reports the misconfiguration', async () => {
      fakeModule(tmpDir, 'nohandle', `
        export const metadata = { prefix: 'nohandle' };
        export const commands = {};
      `);
      const handle = await bootstrap({ modulesDir: tmpDir, autoStartRepl: false, includePackageModules: false });
      const errSpy = jest.spyOn(console, 'error').mockImplementation(() => {});
      await handle.executeCommand('nohandle do-something');
      expect(errSpy).toHaveBeenCalledWith(
        expect.stringContaining('does not have a handle function')
      );
      errSpy.mockRestore();
    });

    test('an error thrown by module.handle is logged and rethrown', async () => {
      fakeModule(tmpDir, 'boom', `
        export const metadata = { prefix: 'boom' };
        export function handle() { throw new Error('handle kaboom'); }
      `);
      const handle = await bootstrap({ modulesDir: tmpDir, autoStartRepl: false, includePackageModules: false });
      const errSpy = jest.spyOn(console, 'error').mockImplementation(() => {});
      await expect(handle.executeCommand('boom go')).rejects.toThrow('handle kaboom');
      expect(errSpy).toHaveBeenCalledWith(
        expect.stringContaining('Error in boom module:'),
        expect.anything()
      );
      errSpy.mockRestore();
    });

    test('a module.handle returning "exit" triggers the shutdown chain', async () => {
      fakeModule(tmpDir, 'bye', `
        export const metadata = { prefix: 'bye' };
        export function handle() { return 'exit'; }
      `);
      const handle = await bootstrap({ modulesDir: tmpDir, autoStartRepl: false, includePackageModules: false });
      let ran = false;
      shutdownService.onShutdown('mark', () => { ran = true; });
      await handle.executeCommand('bye');
  ...
  describe('shutdown', () => {
    ...
    test('farewell is printed and the save-history handler is registered', async () => {
      const handle = await bootstrap({
        modulesDir: tmpDir,
        farewell: 'see you later',
        historyFile: path.join(tmpDir, 'history'),
        autoStartRepl: false,
        includePackageModules: false
      });
      const logSpy = jest.spyOn(console, 'log').mockImplementation(() => {});
      // rl is null in test mode, so save-history is a safe no-op; farewell prints.
      await handle.shutdown('test');
      expect(logSpy.mock.calls.flat().join('\n')).toContain('see you later');
      logSpy.mockRestore();
    });

    test('a second shutdown call is ignored (guarded by isShuttingDown)', async () => {
      const handle = await bootstrap({ modulesDir: tmpDir, autoStartRepl: false, includePackageModules: false });
      let runs = 0;
      shutdownService.onShutdown('counter', () => { runs++; });
      await handle.shutdown('first');
      await handle.shutdown('second');
      expect(runs).toBe(1);
    });

...
export async function bootstrap(options) {
  ...
  async function loadOne(name) {
    const commandsPath = path.join(modulesDir, name, `${name}-commands.js`);
  ...
  if (fs.existsSync(modulesDir)) {
    candidates = fs.readdirSync(modulesDir, { withFileTypes: true })
      .filter(d => d.isDirectory())
      .map(d => d.name);
  }

  const ordered = loadOrder.filter(n => candidates.includes(n));

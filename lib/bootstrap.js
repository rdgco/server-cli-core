...
export async function bootstrap(options) {
  ...
  // ==========================================================================
  // Prefix-aware module lookup
  //
  // `loaded` is keyed by directory name. When a module's
  // `metadata.prefix` differs from its directory name, dispatch by
  // that directory name would fail even though `help` advertises the
  // prefix. Build a second map that includes prefix aliases so the
  // dispatcher resolves what users actually type (which matches what
  // help prints).
  //
  // `loaded` itself stays single-keyed so the init loop, registry, and
  // any consumer iterating `getModules()` see each module exactly once.
  // ==========================================================================

  const loadedByName = { ...loaded };
  for (const [name, mod] of Object.entries(loaded)) {
    const prefix = mod?.metadata?.prefix;
    if (!prefix || prefix === name) continue;
    if (loadedByName[prefix]) {
      throw new Error(
        `[Startup] Module '${name}' declares metadata.prefix='${prefix}' ` +
        `which collides with another loaded module. Pick a different prefix ` +
        'or rename the colliding directory.'
      );
    }
    loadedByName[prefix] = mod;
    logDebug(`[Startup] Aliased module '${name}' as '${prefix}' (from metadata.prefix)`);
  }
  ...
  function completer(line) {
    ...
    if (parts.length === 1 && hasTrailingSpace) {
      ...
      if (!loadedByName[moduleName] && moduleName.endsWith('s')) {
        const singular = moduleName.slice(0, -1);
        if (loadedByName[singular] && moduleHasListCommand(loadedByName[singular])) moduleName = singular;
      }
      const mod = loadedByName[moduleName];
    ...
    if (parts.length === 1 && !hasTrailingSpace) {
      const names = Object.keys(loadedByName);
      const hits = names.filter(n => n.startsWith(parts[0]));
      const pluralHits = [];
      names.forEach(n => {
        const plural = n + 's';
        if (plural.startsWith(parts[0]) && moduleHasListCommand(loadedByName[n])) pluralHits.push(plural);
      });
  ...
  async function handleCommand(input) {
    ...
    const parts = applyPluralRewrite(rawParts, loadedByName);
    if (parts !== rawParts) {
